...
export class buscarGrupoUsuarioController {
  ...
  async buscarGruposUsuario(@Res() response, @Param('idUsuarioDueno') id: string) {
    ...
    const decorator = new LoggerService<string,Iterable<Grupo>>(this.logger,this.buscarGrupoPorUsuario,"Buscar grupos por usuario service: se han buscado los grupos del usuario: " + id);
